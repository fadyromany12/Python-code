"""
AutoPan Node — v5  (Raspberry Pi 4B — Blueprint-Aligned)
=========================================================
Aligned to the Artemis & AutoPan architectural specification.

Key changes from v4:

  [1] ARM KINEMATICS — Full 4-DOF Geometric IK (3-D)
      · q1 (Base)     = atan2(y, x)
      · q3 (Elbow)    = acos((r²+s²-L2²-L3²) / (2·L2·L3))
      · q2 (Shoulder) = atan2(s,r) - atan2(L3·sin(q3), L2+L3·cos(q3))
      · q4 (Wrist)    = DesiredOrientation - (q2+q3)  [gripper stays level]
      · Link lengths from hardware spec:
          d1=150mm (base height), L2=135mm, L3=120mm, L4=83mm
      · Basket at (-100, 0, 200) mm in rover frame

  [2] FSM — 8-stage sequential pipeline per blueprint
      SEARCH → ALIGN → APPROACH → PICKUP →
      LIFT → TURN_TO_BASKET → DROP → RETURN_HOME → (loop)
      · APPROACH stops at STOP_DISTANCE = 350 mm
      · PICKUP: arm descends to Z≈50 mm, gripper closes (PWM 1100)
      · LIFT: arm rises to Z=100 mm to clear ground
      · TURN_TO_BASKET: arm moves to basket (-100, 0, 200) mm
      · DROP: gripper opens (PWM 1500)
      · RETURN_HOME: all joints to 0°, loops back to SEARCH

  [3] VISION — Blueprint-exact distance formula
      · Z = (F_PX × BALL_MM) / bbox_height        [uses height]
      · X = ((cx − W/2) × Z) / F_PX               [lateral offset]
      · Focal length F_PX = 1385.64 px
      · Ball diameter = 67 mm
      · YOLO class 32 ("sports ball"), confidence ≥ 0.40

  [4] SMOOTHING — EMA filter (α = 0.20) for Z and X
      Replaces Kalman filter to match spec exactly.

  [5] SERIAL PROTOCOL — Blueprint format
      Packet: "LeftSpeed RightSpeed Q1 Q2 Q3 Q4 Gripper\\n"
      · LeftSpeed / RightSpeed: signed int [-255..255]
      · Q1..Q4: joint angles in degrees (floats)
      · Gripper: PWM value (1100=closed, 1500=open)

Run modes
---------
    python autopan_v5.py                  # normal
    python autopan_v5.py --no-serial      # vision only
    python autopan_v5.py --port /dev/ttyACM0
    python autopan_v5.py --imgsz 256      # slower Pi
    python autopan_v5.py --debug-hsv
    python autopan_v5.py --calibrate
    python autopan_v5.py --int8           # use INT8 ONNX model

Step 1 (once):  python export_onnx.py
Step 2:         python autopan_v5.py
"""
...
class Config:
    ...
    hsv_ratio_min:   float = 0.30    # raised: blob must be well-covered by mask
    hsv_fast_thresh: float = 0.55    # raised: only skip YOLO for confident hits

    # Shape — strict to keep faces, rectangles, and window strips out
    circularity_min:  float = 0.62   # raised back up from 0.42
    min_box_area:     int   = 400    # ignore tiny noise blobs
    max_box_area:     int   = 40000  # ignore giant blobs (not a ball)
    min_box_px:       int   = 18     # min side length in pixels
    ...
    # Confidence penalty per 10 cm beyond this distance
    dist_conf_decay_per_10cm: float = 0.04
    ...
    lost_frames_limit: int   = 25
    lock_hold_frames:  int   = 20
    ...
    kalman_proc: float = 2.0
    kalman_meas_xy: float = 8.0
    kalman_meas_dist: float = 12.0
...
class BallDetector:
    ...
    def detect(self, frame: np.ndarray) -> List[Detection]:
        ...
        preproc = self._apply_clahe(frame)
        ...
        hsv_dets = self._hsv_blobs(frame, mask)
        ...
        # Submit frame every N frames to YOLO
    ...
    def update(self, dets: List[Detection], ack: bool,
        ...
        with self._lk:
            ...
            # ── APPROACH — P-controller on distance ────────────────────────
                ...
                if target:
                    ...
                    if abs(ctx.cx - cx_frame) > CFG.align_px_thresh * 2:
                        self._transition(State.ALIGN)
                    elif ctx.dist <= CFG.dist_lock_cm:
            ...
            # ── LOCK — hold for N frames ────────────────────────────────────
                ...
                if target:
                    self._sync(target); ctx.lost_frames = 0
                    if ctx.dist > CFG.dist_approach_cm:
                        self._transition(State.APPROACH)
                    else:
                        ctx.lock_frames += 1
                        if ctx.lock_frames >= CFG.lock_hold_frames:
                            # Save ball position for IK
                            ctx.pickup_ball_x = ctx.dist  # rover x = dist forward
...
class AutoPanTracker:
    ...
    def _reset_kalman_if_jumped(self, det: Optional[Detection]):
        ...
        if self._prev_cx is not None and abs(det.cx - self._prev_cx) > 90:
            self._kx.reset(); self._ky.reset(); self._kd.reset()
