"""
AutoPan Node — v16  (Raspberry Pi 4B/5 — Blueprint-Aligned)
============================================================
Builds on v9 with geometry fix and full Tier-3 functional upgrades:
  ...
  [T2-FIX-6] Wheel speeds computed on Pi (P-controller), sent as PWM
      ...
      · APPROACH/LOCK: forward + proportional steer on err_x; base speed
        scales down (soft approach) over approach_brake_cm before lock point.
      · ALIGN: spin-only PD steer.  SEARCH: constant spin_speed rotation.
        All arm states: stop (0, 0).

  [T2-FIX-7] Ball Y-offset (lateral) saved at LOCK → PICKUP transition
      · ball_y_cm = (err_x_px × ctx.dist) / focal_length_px (pinhole).
        [v13/ORG-10] ctx.dist (Kalman-filtered) replaced ctx.dist_ema here.
...
Run modes
---------
    python autopan_v16.py                   # normal
    python autopan_v16.py --no-serial       # vision only
    python autopan_v16.py --port /dev/ttyACM0
    python autopan_v16.py --imgsz 256       # slower Pi
    python autopan_v16.py --debug-hsv
    python autopan_v16.py --calibrate       # MUST re-run on Pi 5 with Pi camera
    python autopan_v16.py --int8            # use INT8 ONNX model
    python autopan_v16.py --target-count 5  # collect 5 balls
    python autopan_v16.py --verbose         # enable DEBUG logging
    python autopan_v16.py --loop            # restart from SEARCH after each SUCCESS

Step 1 (once):  python export_onnx.py
Step 2:         python autopan_v16.py
...
v13  Third audit pass — remaining bugs from v12 review (7 items):
     [ORG-10] pickup_ball_y now uses ctx.dist (Kalman-filtered) instead of
              ctx.dist_ema: both x and y IK inputs now use the same distance
              scale, making the 3-D target geometrically consistent.
     [ORG-11] deposit_release_at sentinel fixed: was reset to 0.0 in the
              on_enter(DEPOSIT) hook; now-0.0≈50000s always exceeded grip_hold_s
              so collected() could fire instantly if gripper entered DEPOSIT open.
              Now initialised to time.monotonic() so the wait starts at entry.
     [ORG-12] IK per-waypoint logs downgraded WARNING → DEBUG. A 10-step
              trajectory to a slightly off-axis ball could generate 3-4 IK
              warnings every PICKUP, drowning real warnings. The on_enter abort
              summary (count/total > 30%) remains at WARNING level.
     [ORG-13] HUD ellipse overlay restored for primary target. In v12 Kalman
              fix ellipse_b was zeroed to prevent best_dist_cm double-averaging,
              which silenced the green ellipse ring. Added ellipse_b_vis field to
              Detection that preserves the pre-Kalman value for rendering only;
              best_dist_cm still reads ellipse_b (zeroed) and is unaffected.
     [YLW-16] Duplicate local `import os` removed from AutoPanTracker.__init__
              — os is already at module level (line 146).
     [YLW-17] Bare `except:` in serial threads (stop(), _tx(), _rx()) replaced
              with `except Exception:` — prevents silently swallowing
              KeyboardInterrupt and SystemExit on Raspberry Pi Ctrl+C.
     [YLW-18] Stale calibration comment updated: autopan_v11.py → autopan_v16.py.
v14  Statue Bug fix + Soft Approach upgrade (2 items):
     [RED-1]  "Statue Bug" — rover sat motionless on startup if no ball was
              immediately visible. Root cause: v12 cold-start fix initialised
              ctx.cx = cam_w/2 → err_x = 0 → steer = 0 → both PWMs = 0 in
              SEARCH, which shared the ALIGN PD formula. Fix: SEARCH and ALIGN
              are now fully decoupled in ArtemisLink.send().
                SEARCH: spins at constant CFG.spin_speed (100 PWM) toward the
                  last known side (err_x ≥ 0 → right, err_x < 0 → left);
                  never stalls on err_x = 0.
                ALIGN:  unchanged PD spin-steer to centre the target.
              New Config fields: drive_min_speed (int=70), spin_speed (int=100).
     [BLU-18] "Soft Approach" — rover was driving at a flat 150 PWM until
              crossing the 15 cm lock line, then stopping abruptly. Now the
              base speed scales linearly from drive_base_speed (≥ 45 cm from
              ball) down to drive_min_speed (at dist_lock_cm) over the last
              30 cm. Prevents skidding on smooth floors and gives the Kalman
              filter time to converge before LOCK is declared.
     NOTE     pickup_ball_y ctx.dist fix (originally tagged as v14 "consistency
              tweak") was already applied in v13 as ORG-10.
v15  Audit pass — 5 fixes (1 functional, 2 stale comments, 1 silent failure,
     1 config hygiene):
     [RED-1]  LOCK state motors stopped — State.LOCK was grouped with
              State.APPROACH in the soft-approach motor branch, so the rover
              continued creeping at drive_min_speed (70 PWM) throughout the
              entire 28-frame lock hold (~2 s at 14 FPS). At 10 cm/s that is
              ~20 cm of additional forward travel — enough to nudge or overrun
              the ball before PICKUP starts. Fix: State.LOCK removed from the
              APPROACH branch; LOCK now falls to else → left = right = 0.
              The rover soft-approaches until lock is declared, then stops and
              holds position for the full hold window. ctx.dist continues to
              update from Kalman each frame while stationary, so the IK target
              at the LOCK→PICKUP transition is still the accurate distance.
     [YLW-2]  Stale dist_ema in module docstring [T2-FIX-7] — said
              "err_x_px × dist_ema" but ctx.dist (Kalman-filtered) has been
              used since ORG-10 (v13). Updated to ctx.dist.
     [YLW-3]  Stale dist_ema in FSMContext inline comment for pickup_ball_y —
              same ORG-10 stale reference; updated to ctx.dist.
     [YLW-4]  SEARCH timeout fires silently — when search_timeout_s elapsed
              with no ball, ctx.entered_at was reset without any log message.
              Impossible to distinguish normal scanning from a stuck SEARCH
              during field debugging. Added log.warning() at the timeout reset.
     [YLW-5]  Hardcoded 0.25 confidence floor in BallDetector.detect() —
              the final valid-detection gate used a magic literal (0.25) while
              every other threshold lives in Config. Extracted to
              CFG.min_final_conf (default 0.25) for full tuneability.
v16  Three QoL upgrades (2 functional, 1 debug):
     [BLU-1]  SEARCH direction reversal — FSMContext.search_spin_dir tracks
              the active spin direction (+1=right, -1=left). on_enter(SEARCH)
              seeds it toward the last-known ball side (err_x sign). Each
              search_timeout_s expiry now flips the direction and logs a warning,
              preventing the rover looping forever over an empty half of the arena.
     [BLU-2]  --loop flag + R key reset — TrackingFSM.reset_for_loop() fully
              resets context and restarts SEARCH. --loop auto-triggers it after
              success_pause_s (default 3 s) of SUCCESS banner hold. The R key in
              the HUD window triggers the same reset at any state (field testing).
     [BLU-3]  --verbose flag — enables DEBUG-level logging at startup without
              editing source. Surfaces IK per-waypoint decisions, Kalman updates,
              and serial receive detail suppressed at INFO level.
──────────────────────────────────────────────────────────────────────────────
"""
...
def ik_4dof(
    ...
    if reach > max_r * 0.97 or reach < min_r * 1.03:
        log.debug("IK: target (%.1f,%.1f,%.1f) unreachable (r=%.1f, max=%.1f)",
    ...
    for val, lo, hi, name in checks:
        if not (lo + margin <= val <= hi - margin):
            log.debug("IK: %s=%.1f° out of limits [%.0f, %.0f]", name, val, lo, hi)
            return None
...
class Config:
    ...
    # [v15] Final detection gate: minimum fused confidence to emit any detection.
    # All three detection paths (YOLO-enriched, HSV-only, hybrid) must clear this.
    # Keep below hsv_solo_min_conf (0.45) so the HSV gate is still the binding
    # constraint for colour-only blobs; this floor catches near-zero junk detections.
    min_final_conf: float = 0.25
    ...
    # WILL differ → re-run:  python autopan_v15.py --calibrate
    ...
    # [v16] Loop mode — restart from SEARCH after SUCCESS
    loop_mode:         bool  = False  # --loop: auto-restart after each SUCCESS
    success_pause_s:   float = 3.0   # seconds to hold SUCCESS banner before loop reset
    ...
    # [v14] Decoupled SEARCH spin + soft-approach floor
    drive_min_speed:  int   = 70     # minimum forward PWM to overcome stall friction;
                                     # soft-approach never goes below this even at dist_lock_cm
    spin_speed:       int   = 100    # constant rotation PWM for blind SEARCH;
                                     # sign sets direction (right by default via err_x >= 0 branch)
    approach_brake_cm: float = 30.0  # distance window over which soft-approach ramps down;
                                     # starts braking from (dist_lock_cm + approach_brake_cm)
...
class Detection:
    ...
    ellipse_a: float = 0.0
    ellipse_b: float = 0.0
    # [v13] Separate HUD-only copy of semi-minor axis.  In AutoPanTracker.run()
    # ellipse_b is zeroed after the Kalman absorbs the fused best_dist_cm, to
    # prevent best_dist_cm from double-counting the ellipse delta.  Zeroing
    # ellipse_b also silenced the green ellipse overlay in HUD.draw() because
    # the draw guard checks ellipse_b > 0.  This field preserves the original
    # value for rendering only; best_dist_cm never references it.
    ellipse_b_vis: float = 0.0
    ...
    def detect(self, frame: np.ndarray) -> List[Detection]:
        ...
        for d in fused:
            ...
            if (d.circularity >= CFG.circularity_min
                    ...
                    and d.confidence >= CFG.min_final_conf):   # [v15] was magic literal 0.25
...
class FSMContext:
    ...
    pickup_ball_y:   float = 0.0   # lateral offset  at pickup (rover frame, cm)
                                    # y = (err_x_px × ctx.dist) / focal_length_px
                                    # [v13/ORG-10] uses ctx.dist (Kalman-filtered),
                                    # not ctx.dist_ema — both IK axes now consistent.
    search_spin_dir: int   = 1     # [v16] +1=spin right, -1=spin left; seeded by
                                    # on_enter(SEARCH) toward last-known ball side,
                                    # flipped on each search_timeout_s expiry.
...
class TrackingFSM:
        ...
        with self._lk:
            ...
            if ctx.state == State.SEARCH:
                if elapsed > CFG.search_timeout_s and not target:
                    ctx.search_spin_dir *= -1   # [v16] alternate half each lap
                    log.warning(
                        "SEARCH: no ball found after %.1fs — reversing spin to %s.",
                        elapsed, "right" if ctx.search_spin_dir > 0 else "left")
                ...
                if target:
                    ...
                    if ctx.dist_ema > CFG.dist_approach_cm:
                        ...
                        if ctx.lock_frames >= CFG.lock_hold_frames:
                            ...
                            ctx.pickup_ball_y = (err_x_px * ctx.dist) / CFG.focal_length_px
                            # [v13] use ctx.dist (Kalman-filtered) not ctx.dist_ema —
                            # dist_ema still reflects approach-phase lag at this moment;
                            # using a different distance scale for x vs y produces an
                            # off-axis IK target that is not geometrically consistent.
    ...
    def reset_for_loop(self):
        """
        [v16] Fully reset FSM context for a new collection run.
        Called by --loop mode after SUCCESS and by the R key in the HUD.
        """
        ctx = self.ctx
        ctx.cx                = float(CFG.cam_w // 2)  # prevent cold-start err_x spike
        ctx.cy                = 0.0
        ctx.dist              = 0.0
        ctx.dist_raw          = 0.0
        ctx.dist_ema          = 0.0
        ctx.collected         = 0
        ctx.n_balls_visible   = 0
        ctx.det_confirm_count = 0
        ctx.arm_traj          = []
        ctx.arm_step          = 0
        ctx.arm_step_at       = 0.0
        ctx.deposit_release_at = 0.0
        ctx.pickup_grip_at    = 0.0
        ctx.gripper_closed    = False
        ctx.pickup_ball_x     = 0.0
        ctx.pickup_ball_y     = 0.0
        ctx.lost_frames       = 0
        ctx.lock_frames       = 0
        # _transition fires on_enter(SEARCH) which re-seeds search_spin_dir
        self._transition(State.SEARCH)
...
def _(f, c):
    # [v16] Spin toward last-known ball side; cold start: cx=cam_w/2 → err_x=0 → right
    err_x = c.cx - CFG.cam_w / 2
    c.search_spin_dir = 1 if err_x >= 0 else -1
    log.info("SEARCH — scanning for ball (spinning %s).",
             "right" if c.search_spin_dir > 0 else "left")
...
def _(f, c):
    ...
    c.arm_step          = 0
    c.arm_step_at       = time.monotonic()
    c.deposit_release_at = time.monotonic()  # [v13] start clock at entry; was 0.0
                                              # (sentinel): now-0.0≈50000s is always
                                              # >= grip_hold_s, so collected would fire
                                              # instantly if gripper happened to be open.
...
class ArtemisLink:
    ...
    def send(self, ctx: FSMContext):
        ...
        # ── [v14] Decoupled motor logic: SEARCH / ALIGN / APPROACH / LOCK ────────
        #
        # SEARCH — blind constant-speed spin toward the last known side.
        #   Before v14, SEARCH and ALIGN shared the PD steer formula.  After the
        #   v12 cold-start fix (ctx.cx = cam_w/2), err_x = 0 on startup, so
        #   steer = 0 and both PWMs = 0 → the rover sat motionless forever
        #   ("Statue Bug").  SEARCH now spins at a fixed CFG.spin_speed;
        #   direction is toward the half of the frame where the last detection
        #   was seen (err_x ≥ 0 → spin right, err_x < 0 → spin left).
        #
        # ALIGN — PD controller to zero the centring error (unchanged).
        #
        # APPROACH — "Soft Approach": base speed scales linearly from
        #   drive_base_speed (at or beyond approach_brake_cm) down to
        #   drive_min_speed (at dist_lock_cm), preventing skidding and giving
        #   the Kalman filter time to converge before LOCK is declared.
        #
        # LOCK — [v15] wheels STOP. The rover soft-approaches until lock is
        #   first declared, then holds position for lock_hold_frames. ctx.dist
        #   keeps updating from Kalman while stationary, so the IK target saved
        #   at the LOCK→PICKUP transition is the accurate distance. Grouping LOCK
        #   with APPROACH meant the rover crept at 70 PWM for ~2 s (28 frames @
        #   14 FPS) and could nudge/overrun the ball before PICKUP started.
        if ctx.state == State.APPROACH:
            # Soft Approach: interpolate base speed over the last approach_brake_cm
            dist_buffer  = max(ctx.dist - CFG.dist_lock_cm, 0.0)
            speed_factor = _clamp(dist_buffer / CFG.approach_brake_cm, 0.0, 1.0)
            base_pwm     = int(CFG.drive_min_speed +
                               (CFG.drive_base_speed - CFG.drive_min_speed) * speed_factor)
            steer        = CFG.drive_kp * err_x + CFG.drive_kd * derr_x
            left_pwm     = int(_clamp(base_pwm + steer, 0, 255))
            right_pwm    = int(_clamp(base_pwm - steer, 0, 255))

        elif ctx.state == State.ALIGN:
            # Centre the ball with PD spin — no forward motion
            steer     = CFG.drive_kp * err_x + CFG.drive_kd * derr_x
            left_pwm  = int(_clamp( steer, -255, 255))
            right_pwm = int(_clamp(-steer, -255, 255))

        elif ctx.state == State.SEARCH:
            # [v16] direction tracked in FSMContext; flips each search_timeout_s expiry
            spin_dir  = ctx.search_spin_dir
            left_pwm  =  CFG.spin_speed * spin_dir
            right_pwm = -CFG.spin_speed * spin_dir

        else:
            left_pwm = right_pwm = 0
            self._prev_err_x = None   # [v11] reset: derivative skipped on next movement start
...
class HUD:
    ...
    def draw(cls, frame: np.ndarray, ctx: FSMContext,
        ...
        for i, d in enumerate(dets):
            ...
            # [v13] Use ellipse_b_vis (preserved pre-Kalman value) — ellipse_b is
            # zeroed in run() to prevent best_dist_cm double-counting.
            if d.ellipse_a > 0 and d.ellipse_b_vis > 0:
                ex = int(d.cx); ey = int(d.cy)
                cv2.ellipse(frame, (ex, ey),
                            (int(d.ellipse_a), int(d.ellipse_b_vis)),
                            0, 0, 360, (0, 200, 100), 1)
...
class AutoPanTracker:
    def __init__(self):
        ...
        self._success_at: Optional[float] = None   # [v16] loop-mode pause timer
    ...
    def run(self):
        ...
        log.info("AutoPan v16 running. Press Q to quit, R to reset.")
        ...
        try:
            while not self._done.is_set():
                ...
                if target and dets:
                    ...
                    # [v13] Preserve original ellipse_b in ellipse_b_vis so HUD.draw()
                    # can still render the green ellipse ring for the primary target.
                    ...
                    dets[0] = dataclasses.replace(
                        ...
                        ellipse_b    = 0.0,              # prevent double-averaging in best_dist_cm
                        ellipse_b_vis = target.ellipse_b, # keep for HUD overlay
                    )
                ...
                # [v16] --loop: hold SUCCESS banner then auto-restart
                if CFG.loop_mode and self._ctx.state == State.SUCCESS:
                    if self._success_at is None:
                        self._success_at = time.monotonic()
                    elif time.monotonic() - self._success_at >= CFG.success_pause_s:
                        self._success_at = None
                        log.info("Loop mode — restarting from SEARCH.")
                        self._fsm.reset_for_loop()
                        self._kx.reset(); self._ky.reset(); self._kd.reset()

                HUD.draw(frame, self._ctx, dets, self._fps, self._comm.connected)
                cv2.imshow("AutoPan v16", frame)
                key = cv2.waitKey(1) & 0xFF
                if key == ord("q"):
                    self._done.set()
                elif key == ord("r"):   # [v16] R key: manual reset at any state
                    log.info("Manual reset (R key) — restarting from SEARCH.")
                    self._success_at = None
                    self._fsm.reset_for_loop()
                    self._kx.reset(); self._ky.reset(); self._kd.reset()
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="AutoPan v16")
    ...
    ap.add_argument("--loop",         action="store_true",
                    help="Restart from SEARCH after each SUCCESS (multi-run testing)")
    ap.add_argument("--verbose",      action="store_true",
                    help="Enable DEBUG-level logging (IK, Kalman, serial detail)")
    args = ap.parse_args()
