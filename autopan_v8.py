"""
AutoPan Node — v8  (Raspberry Pi 4B/5 — Blueprint-Aligned)
===========================================================
Builds on v7 (= v6 + TFLite engine) with three targeted bug-fixes:

  [FIX-1] TENNIS BALL ONLY — eliminate pool ball / glare false positives
      · HSV bands 2 & 3: saturation floor raised (60→90, 50→80).
        Cream/beige/pool-ball hues are heavily desaturated (sat<80 in HSV);
        tennis felt stays above sat≈90 even on a worn ball.
      · HSV-solo confidence gate (hsv_solo_min_conf=0.45): HSV-only blobs
        that YOLO never confirmed are silently dropped.  A real tennis ball
        almost always triggers both YOLO class-32 AND HSV; a stray object
        (pool ball, yellow wall strip, skin) typically triggers only one.
      · Single-target output: detect() now returns valid[:1].
        Previously returning 2-3 detections caused "BALL 3" on the HUD and
        let the FSM lock onto non-ball secondary objects.

  [FIX-2] WORKING RANGE ON PI 5 — camera resolution mismatch diagnosis
      · CameraStream logs actual vs requested resolution at startup.
        On Pi 5 / libcamera, CAP_PROP_FRAME_WIDTH/HEIGHT requests can be
        silently ignored, delivering a different resolution than 640×480.
        This corrupts focal_length_px → every dist_cm is wrong → range
        appears limited to ~40 cm.
      · A clear WARNING is printed if actual ≠ requested, with instructions
        to re-run --calibrate at the real resolution.
      · Corrected Brio 100 dFoV comment: 58° (not ~70° — that was wrong).

  [FIX-3] FOCAL LENGTH DOC / COMMENT FIX
      · focal_length_px comment now documents 58° dFoV (the true Brio 100
        spec) and explains why re-calibration is required on different hardware.

Run modes
---------
    python autopan_v8.py                  # normal
    python autopan_v8.py --no-serial      # vision only
    python autopan_v8.py --port /dev/ttyACM0
    python autopan_v8.py --imgsz 256      # slower Pi
    python autopan_v8.py --debug-hsv
    python autopan_v8.py --calibrate      # MUST re-run on Pi 5 with Pi camera
    python autopan_v8.py --int8           # use INT8 ONNX model

Step 1 (once):  python export_onnx.py
Step 2:         python autopan_v8.py

─────── Changelog vs v7 ──────────────────────────────────────────────────────
v7  (= v6 + TFLite engine, stability tuning)
v8  hsv_bands[1,2] sat floors +30/+30; hsv_solo_min_conf gate; single-target
    return; camera resolution mismatch WARNING; 58° dFoV comment correction.
──────────────────────────────────────────────────────────────────────────────
"""
