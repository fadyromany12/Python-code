"""
AutoPan Node — v15  (Raspberry Pi 4B/5 — Blueprint-Aligned)
============================================================
...
Run modes
---------
    python autopan_v15.py                   # normal
    python autopan_v15.py --no-serial       # vision only
    python autopan_v15.py --port /dev/ttyACM0
    python autopan_v15.py --imgsz 256       # slower Pi
    python autopan_v15.py --debug-hsv
    python autopan_v15.py --calibrate       # MUST re-run on Pi 5 with Pi camera
    python autopan_v15.py --int8            # use INT8 ONNX model
    python autopan_v15.py --target-count 5  # collect 5 balls

Step 1 (once):  python export_onnx.py
Step 2:         python autopan_v15.py
...
v13  Third audit pass — remaining bugs from v12 review (7 items):
     ...
     [YLW-18] Stale calibration comment updated: autopan_v11.py → autopan_v13.py.
        ...
        with self._lk:
            ...
            if ctx.state == State.SEARCH:
                if elapsed > CFG.search_timeout_s and not target:
                    log.warning(              # [v15] was silent — impossible to debug
                        "SEARCH: no ball found after %.1fs — keep scanning.", elapsed)
...
class ArtemisLink:
    ...
    def send(self, ctx: FSMContext):
        ...
        if ctx.state == State.APPROACH:
            ...
            # Spin at constant speed toward last known side (or right on cold start)
            spin_dir  = 1 if err_x >= 0 else -1
...
class AutoPanTracker:
    ...
    def run(self):
        ...
        log.info("AutoPan v15 running. Press Q to quit.")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="AutoPan v15")
