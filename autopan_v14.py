"""
AutoPan Node — v14  (Raspberry Pi 4B/5 — Blueprint-Aligned)
============================================================
...
Run modes
---------
    python autopan_v14.py                   # normal
    python autopan_v14.py --no-serial       # vision only
    python autopan_v14.py --port /dev/ttyACM0
    python autopan_v14.py --imgsz 256       # slower Pi
    python autopan_v14.py --debug-hsv
    python autopan_v14.py --calibrate       # MUST re-run on Pi 5 with Pi camera
    python autopan_v14.py --int8            # use INT8 ONNX model
    python autopan_v14.py --target-count 5  # collect 5 balls

Step 1 (once):  python export_onnx.py
Step 2:         python autopan_v14.py
...
class Config:
    ...
    # WILL differ → re-run:  python autopan_v14.py --calibrate
...
class ArtemisLink:
    ...
    def send(self, ctx: FSMContext):
        ...
        #
        # APPROACH / LOCK — "Soft Approach": base speed scales linearly from
        #   drive_base_speed (at or beyond 30 cm) down to drive_min_speed (at
        #   dist_lock_cm), preventing skidding and improving lock precision.
        if ctx.state in (State.APPROACH, State.LOCK):
            # Soft Approach: interpolate base speed over the last 30 cm
...
class AutoPanTracker:
    ...
    def run(self):
        ...
        log.info("AutoPan v14 running. Press Q to quit.")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="AutoPan v14")
