...
===========================================================
Builds on v8 with full Tier-1 crash fixes and Tier-2 functional upgrades:
...
Run modes
---------
    python autopan_v9.py                  # normal
    python autopan_v9.py --no-serial      # vision only
    python autopan_v9.py --port /dev/ttyACM0
    python autopan_v9.py --imgsz 256      # slower Pi
    python autopan_v9.py --debug-hsv
    python autopan_v9.py --calibrate      # MUST re-run on Pi 5 with Pi camera
    python autopan_v9.py --int8           # use INT8 ONNX model

Step 1 (once):  python export_onnx.py
Step 2:         python autopan_v9.py
            ...
            # ── PICKUP — execute arm trajectory step by step ────────────────
                ...
                if not ctx.arm_traj:
                    ...
                    if ctx.arm_step < len(ctx.arm_traj):
                        ...
                        # Trajectory complete: close gripper, then LIFT arm
                        if not ctx.gripper_closed:
                            ...
                            self._transition(State.LIFT)  # [v9] was State.DEPOSIT
...
class ArtemisLink:
    ...
    def send(self, ctx: FSMContext):
        ...
        if ctx.state in (State.APPROACH, State.LOCK):
            ...
            left_pwm = right_pwm = 0   # arm states: motors stopped
...
class AutoPanTracker:
    ...
    def run(self):
        ...
        log.info("AutoPan v9 running. Press Q to quit.")
