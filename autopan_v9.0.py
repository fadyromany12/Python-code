"""
AutoPan Node — v9  (Raspberry Pi 4B/5 — Blueprint-Aligned)
===========================================================
Fixes all Tier-1 crash bugs and Tier-2 functional gaps discovered
after the v6→v7 4-DOF arm upgrade was never fully propagated:

  [CRASH-1] build_pickup_trajectory / build_deposit_trajectory used
            deleted ArmParams attributes (shoulder_x, shoulder_z, l1,
            box_x, box_z) → AttributeError on first PICKUP.
            → Replaced with full 3-D trajectory builders using ik_4dof().

  [CRASH-2] Both old trajectory functions called ik_2dof() which no longer
            exists → NameError on first PICKUP.
            → All trajectory code now calls ik_4dof(x, y, z).

  [CRASH-3] ArtemisLink.send() unpacked arm angles as (sh, el, wr) — a
            3-tuple — but ik_4dof returns (q1, q2, q3, q4) — 4-tuple
            → ValueError: too many values to unpack on first arm step.
            → Fixed to unpack q1, q2, q3, q4 correctly.

  [CRASH-4] Serial packet sent only 3 joint angles (sh, el, wr).
            Arduino never received base-rotation (q1); base servo never moved.
            → Protocol updated to blueprint format:
              "LeftSpeed RightSpeed Q1 Q2 Q3 Q4 Gripper\\n"

  [CRASH-5] run_ik_test() still called ik_2dof() → NameError.
            → Updated to call ik_4dof(x, y, z).

  [FUNC-1]  Wheel speeds computed on Pi side (P-controller) and sent
            directly as LeftSpeed/RightSpeed so Arduino just executes.

  [FUNC-2]  Ball lateral Y offset saved at LOCK→PICKUP so the base
            servo (q1) actually aims at the real ball position, not the
            rover's forward axis.
              ball_y_cm = −(err_x_px × dist_cm) / focal_length_px

  [FUNC-3]  LIFT state added between PICKUP and DEPOSIT.
            Arm rises to safe carry height before swinging to basket,
            eliminating the risk of dragging the ball across the ground.

  [FUNC-4]  RETURN_HOME state added after DEPOSIT.
            Arm sweeps back to safe park before VERIFY/SEARCH,
            so the next pickup trajectory starts from a known position.

  [FUNC-5]  Pickup trajectory retry: if >30%% of waypoints returned by
            ik_4dof() are None (unreachable), PICKUP aborts back to
            SEARCH rather than stalling silently on None steps.

─────── Changelog ────────────────────────────────────────────────────────────
v8  HSV sat-floor fix; single-target output; camera resolution WARNING
v9  4-DOF trajectory rebuild; serial protocol fix; LIFT+RETURN_HOME states;
    motor speed on Pi; ball Y offset; pickup retry gate.
──────────────────────────────────────────────────────────────────────────────
"""
...
def _cartesian_interpolate_3d(
    start: Tuple[float, float, float],
    end:   Tuple[float, float, float],
    steps: int = 10,
) -> List[Tuple[float, float, float]]:
    """Straight-line interpolation through 3-D Cartesian space."""
    n = max(steps, 2)
    return [
        (
            start[0] + i / (n - 1) * (end[0] - start[0]),
            start[1] + i / (n - 1) * (end[1] - start[1]),
            start[2] + i / (n - 1) * (end[2] - start[2]),
        )
        for i in range(n)
    ]
...
def _build_3d_trajectory(
    ...
    """
    Interpolate between 3-D Cartesian waypoints and solve ik_4dof at each point.
    steps_list[i] controls the number of interpolation steps between waypoint i
    and i+1.  Returns a flat list of (q1,q2,q3,q4) tuples; None = IK failed.
    """
    traj: List[Optional[Tuple[float, float, float, float]]] = []
    for i, (start, end) in enumerate(zip(waypoints, waypoints[1:])):
        n = steps_list[i] if i < len(steps_list) else 10
        for pt in _cartesian_interpolate_3d(start, end, n):
            traj.append(ik_4dof(*pt))
    return traj


# ── Safe arm park position (rover frame, cm) ──────────────────────────────────
# Used as the start/end of every arm motion.  The arm points slightly forward
# and high — clear of the rover body, safe while driving.
_ARM_HOME: Tuple[float, float, float] = (14.0, 0.0, 22.0)
...
def build_pickup_trajectory(
    ...
    """
    Home → Hover (8 cm above ball) → Grasp (ball centre height).
    [v9-fix] Uses ik_4dof(x, y, z) and correct ArmParams attributes.
    ball_y_cm accounts for lateral offset so q1 (base) aims at the real ball.
    """
    hover = (ball_x_cm, ball_y_cm, ARM.pickup_z_cm + 8.0)
    grasp = (ball_x_cm, ball_y_cm, ARM.pickup_z_cm)
    return _build_3d_trajectory(
        [_ARM_HOME, hover, grasp],
        [6, steps, max(steps // 2, 3)],
    )
...
def build_lift_trajectory(
    ...
    """
    [v9-new] LIFT state: raise arm from grasp height to safe carry height
    before the base servo swings toward the basket.  Prevents dragging the
    ball across the ground during the deposit swing.
    """
    grasp  = (ball_x_cm, ball_y_cm, ARM.pickup_z_cm)
    lifted = (ball_x_cm, ball_y_cm, ARM.pickup_z_cm + 10.0)
    return _build_3d_trajectory([grasp, lifted], [steps])
...
def build_deposit_trajectory(
    ...
    """
    [v9-fix] Carry height → basket position.
    Uses ARM.basket_x/y/z (correct v7+ attribute names).
    """
    lifted = (ball_x_cm, ball_y_cm, ARM.pickup_z_cm + 10.0)
    basket = (ARM.basket_x, ARM.basket_y, ARM.basket_z)
    return _build_3d_trajectory([lifted, basket], [steps])


def build_return_home_trajectory(steps: int = 8
                                 ) -> List[Optional[Tuple[float, float, float, float]]]:
    """
    [v9-new] RETURN_HOME state: basket → safe park before the next SEARCH.
    Ensures every pickup trajectory starts from a known arm position.
    """
    basket = (ARM.basket_x, ARM.basket_y, ARM.basket_z)
    # Rise above basket first to avoid clipping the rover body on the way back
    mid    = (0.0, 0.0, ARM.d1 + ARM.L2)
    return _build_3d_trajectory([basket, mid, _ARM_HOME], [steps, steps])
...
class Config:
    ...
    # WILL differ → re-run:  python autopan_v8.py --calibrate
    ...
    # [v9] Motor control — wheel speeds computed on Pi, sent as signed PWM [-255,255]
    # Arduino receives LeftSpeed and RightSpeed and drives BTS7960 directly.
    spin_speed:     int   = 100   # SEARCH: spin-in-place PWM
    turn_speed_max: int   = 150   # ALIGN:  max in-place turn PWM
    approach_speed: int   = 120   # APPROACH: base forward PWM
    steer_kp:       float = 0.50  # P-gain: err_x (px) → steer offset (PWM units)
...
# ══════════════════════════════════════════════════════════════════════════════
class State(Enum):
    SEARCH      = auto()   # Spin in place; find ball
    ALIGN       = auto()   # Pan until X error < threshold
    APPROACH    = auto()   # Drive forward until dist_lock_cm
    LOCK        = auto()   # Hold N frames; confirm ball position
    PICKUP      = auto()   # Arm descends to ball; gripper closes
    LIFT        = auto()   # [v9-new] Arm rises before swinging to basket
    DEPOSIT     = auto()   # Arm swings to basket; gripper opens
    RETURN_HOME = auto()   # [v9-new] Arm returns to safe park position
    VERIFY      = auto()   # Re-scan for more balls
    SUCCESS     = auto()
...
class FSMContext:
    ...
    # [v9] Wheel motor outputs — computed on Pi, sent directly to Arduino
    left_pwm:        int   = 0    # left  wheel PWM [-255, 255]
    right_pwm:       int   = 0    # right wheel PWM [-255, 255]
    ...
    pickup_ball_x:   float = 0.0   # saved pickup position X (rover frame, cm)
    pickup_ball_y:   float = 0.0   # [v9] saved pickup position Y (lateral, cm)
...
class TrackingFSM:
    ...
    def update(self, dets: List[Detection], ack: bool,
        ...
        with self._lk:
            ...
            # ── SEARCH — spin in place until ball confirmed ──────────────────
            if ctx.state == State.SEARCH:
                ctx.left_pwm  = +CFG.spin_speed
                ctx.right_pwm = -CFG.spin_speed
            ...
            # ── ALIGN — pan until X error < threshold ────────────────────────
            elif ctx.state == State.ALIGN:
                if target:
                    self._sync(target); ctx.lost_frames = 0
                    err_x = ctx.cx - cx_frame
                    spd   = int(_clamp(abs(err_x) * CFG.steer_kp,
                                       30, CFG.turn_speed_max))
                    # Ball right of centre → turn right (left fwd, right back)
                    ctx.left_pwm  = +spd if err_x > 0 else -spd
                    ctx.right_pwm = -spd if err_x > 0 else +spd
                    if abs(err_x) <= CFG.align_px_thresh:
                        self._transition(State.APPROACH)
                else:
                    ctx.left_pwm = ctx.right_pwm = 0
                    ctx.lost_frames += 1
            ...
            # ── APPROACH — P-controller on distance ──────────────────────────
                ...
                if target:
                    self._sync(target); ctx.lost_frames = 0
                    err_x   = ctx.cx - cx_frame
                    steer   = int(_clamp(err_x * CFG.steer_kp, -50, 50))
                    ctx.left_pwm  = int(_clamp(CFG.approach_speed + steer, 0, 255))
                    ctx.right_pwm = int(_clamp(CFG.approach_speed - steer, 0, 255))
                    if abs(err_x) > CFG.align_px_thresh * 2:
                        self._transition(State.ALIGN)
                    ...
                    ctx.lost_frames += 1
                    if (ctx.lost_frames > CFG.lost_frames_limit
                            or elapsed > CFG.search_timeout_s):
                        self._transition(State.SEARCH)

            # ── LOCK — hold N frames; record ball position ───────────────────
                ...
                if target:
                    ...
                    if ctx.dist_ema > CFG.dist_approach_cm:
                        ...
                        if ctx.lock_frames >= CFG.lock_hold_frames:
                            # [v9] Save 3-D ball position for IK:
                            #   X = forward distance (EMA-smoothed, cm)
                            #   Y = lateral offset — pinhole projection
                            #       +Y = left (rover frame), +err_x = ball right
                            ctx.pickup_ball_x = ctx.dist_ema
                            ctx.pickup_ball_y = (
                                -(ctx.cx - cx_frame)
                                * ctx.dist_ema
                                / CFG.focal_length_px
            ...
            # ── PICKUP — arm descends to ball, gripper closes ────────────────
            elif ctx.state == State.PICKUP:
                ctx.left_pwm = ctx.right_pwm = 0
                # Empty trajectory = retry gate fired in enter hook → abort
                if not ctx.arm_traj:
                    self._transition(State.SEARCH)
            ...
            # ── LIFT — [v9-new] raise arm before swinging to basket ──────────
            ...
            # ── DEPOSIT — arm swings to basket, gripper opens ────────────────
            ...
            # ── RETURN_HOME — [v9-new] arm returns to park before next search
            elif ctx.state == State.RETURN_HOME:
                ctx.left_pwm = ctx.right_pwm = 0
                if now - ctx.arm_step_at >= CFG.deposit_step_s:
                    ctx.arm_step_at = now
                    if ctx.arm_step < len(ctx.arm_traj):
...
def _(f, c): log.info("SEARCH — spinning to find ball.")

@TrackingFSM.on_enter(State.ALIGN)
def _(f, c): log.info("ALIGN — centering ball.")

@TrackingFSM.on_enter(State.APPROACH)
def _(f, c): log.info("APPROACH — driving, dist_ema=%.1f cm.", c.dist_ema)

@TrackingFSM.on_enter(State.LOCK)
def _(f, c): log.info("LOCK — holding at %.1f cm.", c.dist_ema)

@TrackingFSM.on_enter(State.PICKUP)
def _(f, c):
    log.info("PICKUP — ball at (x=%.1f cm, y=%.1f cm). Building 4-DOF trajectory.",
             c.pickup_ball_x, c.pickup_ball_y)
    traj = build_pickup_trajectory(c.pickup_ball_x, c.pickup_ball_y)
    # [v9] Retry gate: if >30% of waypoints are IK-unreachable, abort rather
    # than stalling silently on None steps.  Likely cause: stop distance was
    # too far — re-calibrate dist_lock_cm or camera focal length.
    none_count = sum(1 for wp in traj if wp is None)
    if traj and none_count / len(traj) > 0.30:
        log.warning(
            "PICKUP: %.0f%% of %d waypoints unreachable — aborting to SEARCH. "
            "Check dist_lock_cm (%.1f cm) and focal_length_px (%.1f).",
            100.0 * none_count / len(traj), len(traj),
            CFG.dist_lock_cm, CFG.focal_length_px,
        )
        c.arm_traj = []    # empty → FSM.update() transitions back to SEARCH
    else:
        c.arm_traj = traj
    c.arm_step       = 0
    c.arm_step_at    = time.monotonic()
    c.gripper_closed = False
    log.info("  trajectory: %d waypoints, %d None.", len(traj), none_count)

@TrackingFSM.on_enter(State.LIFT)
def _(f, c):
    log.info("LIFT — raising arm to carry height before deposit sweep.")
    traj = build_lift_trajectory(c.pickup_ball_x, c.pickup_ball_y)
    c.arm_traj    = traj
    c.arm_step    = 0
    c.arm_step_at = time.monotonic()
...
def _(f, c):
    log.info("DEPOSIT — moving arm to basket. Collected so far: %d.", c.collected)
    traj = build_deposit_trajectory(c.pickup_ball_x, c.pickup_ball_y)
    c.arm_traj    = traj
    c.arm_step    = 0
    c.arm_step_at = time.monotonic()
    log.info("  trajectory: %d waypoints.", len(traj))

@TrackingFSM.on_enter(State.RETURN_HOME)
def _(f, c):
    log.info("RETURN_HOME — arm returning to park position.")
    traj = build_return_home_trajectory()
    c.arm_traj    = traj
    c.arm_step    = 0
    c.arm_step_at = time.monotonic()
    log.info("  trajectory: %d waypoints.", len(traj))
...
_STATE_ID = {
    ...
    State.PICKUP: 4, State.LIFT: 5, State.DEPOSIT: 6,
    State.RETURN_HOME: 7, State.VERIFY: 8, State.SUCCESS: 9,
}
...
class ArtemisLink:
    ...
    def send(self, ctx: FSMContext):
        ...
        # [v9-fix] Extract all 4 joint angles from the current trajectory step.
        # Previously unpacked only 3 (sh, el, wr) → ValueError on ik_4dof's 4-tuple.
        q1 = q2 = q3 = q4 = 0.0
        gripper = 1 if ctx.gripper_closed else 0
        _ARM_ACTIVE = {State.PICKUP, State.LIFT, State.DEPOSIT, State.RETURN_HOME}
        if ctx.state in _ARM_ACTIVE and ctx.arm_traj:
            idx    = min(ctx.arm_step, len(ctx.arm_traj) - 1)
            angles = ctx.arm_traj[idx]
            if angles is not None:
                q1, q2, q3, q4 = angles   # [v9-fix] unpack 4-tuple, not 3

        # [v9-fix] Protocol per blueprint:
        #   "LeftSpeed RightSpeed Q1 Q2 Q3 Q4 Gripper\n"
        # Motor speeds are now computed on the Pi (P-controller) and sent
        # directly — Arduino just maps them to BTS7960 PWM, no logic needed.
        cmd = (
            f"{ctx.left_pwm} {ctx.right_pwm} "
...
class HUD:
    ...
    _PC = {
        ...
        State.LIFT:        (  0, 240, 130),   # green-cyan  [v9-new]
        State.DEPOSIT:     (200, 120, 255),
        State.RETURN_HOME: (150, 150, 255),   # lavender    [v9-new]
    ...
    def draw(cls, frame: np.ndarray, ctx: FSMContext,
        ...
        # PICKUP / LIFT / DEPOSIT / RETURN_HOME arm progress bar
        _ARM_STATES = {State.PICKUP, State.LIFT, State.DEPOSIT, State.RETURN_HOME}
        if ctx.state in _ARM_STATES and ctx.arm_traj:
...
class AutoPanTracker:
    ...
    def run(self):
        ...
        log.info("AutoPan v8 running. Press Q to quit.")
