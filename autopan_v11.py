"""
AutoPan Node — v11  (Raspberry Pi 4B/5 — Blueprint-Aligned)
============================================================
...
Run modes
---------
    python autopan_v11.py                   # normal
    python autopan_v11.py --no-serial       # vision only
    python autopan_v11.py --port /dev/ttyACM0
    python autopan_v11.py --imgsz 256       # slower Pi
    python autopan_v11.py --debug-hsv
    python autopan_v11.py --calibrate       # MUST re-run on Pi 5 with Pi camera
    python autopan_v11.py --int8            # use INT8 ONNX model
    python autopan_v11.py --target-count 5  # collect 5 balls

Step 1 (once):  python export_onnx.py
Step 2:         python autopan_v11.py
...
v11  Systematic audit fixes (all 17 items):
     [RED-1]  DEPOSIT infinite loop fixed: separate deposit_release_at timer so
              the release-wait check is never measured against the just-reset
              arm_step_at; magic 0.4 literal replaced with CFG.grip_hold_s.
     [RED-2]  LIFT + DEPOSIT IK abort gate added (mirrors PICKUP gate): if
              >30% of waypoints are None the trajectory is cleared and FSM
              falls back to APPROACH, preventing silent 0,0,0,0 replay.
     [RED-3]  TFLite max_box_area filter added: close-range giant blob (>160k
              px²) no longer slips through TFLite while being caught by ONNX.
     [RED-4]  collect_timeout_s now wired in PICKUP, LIFT, DEPOSIT: default
              raised to 30 s (covers full 15-step pickup at 0.8 s/step);
              hard-jammed arm or disconnected serial escapes via APPROACH.
     [ORG-5]  PD cold-start spike eliminated: _prev_err_x initialised as None;
              derivative skipped on first send() call and after motor-idle reset.
     [ORG-6]  downscale() now syncs CFG.cam_w / CFG.cam_h: err_x centre and
              align_px_thresh use the correct frame centre after FPS downsample.
     [ORG-7]  strong_hsv fast-path activated: YOLO submission skipped when a
              high-confidence HSV+circularity hit exists (was computed but
              immediately discarded — YOLO ran unconditionally every frame).
     [ORG-8]  Grasp verify now uses target.best_dist_cm (avg of bbox + ellipse)
              instead of raw target.dist_cm; matches the rest of the codebase
              and is more accurate at close range where ellipse estimate matters.
     [YLW-9]  Dead field arm_phase removed (was never written).
     [YLW-10] Dead field _last_target_id removed (was never populated).
     [YLW-11] _STATE_ID dict deleted (not referenced since v9 removed it from
              the serial packet).
     [YLW-12] ACK wired: PICKUP/LIFT/DEPOSIT log DEBUG when Arduino confirms;
              ack param in update() no longer silently ignored.
     [BLU-13] ArmParams docstring: basket_y comment corrected to 20 mm → 2.0 cm.
     [BLU-14] focal_length_px comment: calibrate reference updated to v11.
     [BLU-15] ArtemisLink docstring: version tag updated to v11.
     [BLU-16] strong_hsv comment aligned with the now-active fast-path code.
──────────────────────────────────────────────────────────────────────────────
"""
                ...
                if target:
                    ...
                    if ctx.dist_ema > CFG.dist_approach_cm:
                        ...
                        if ctx.lock_frames >= CFG.lock_hold_frames:
                            ...
                            ctx.pickup_ball_x = ctx.dist_ema          # forward (cm)
            ...
            # ── PICKUP — execute arm trajectory step by step ────────────────
                ...
                if not ctx.arm_traj:
                    ...
                    if ctx.arm_step < len(ctx.arm_traj):
                        ...
                        # Trajectory complete: close gripper, then verify grasp
                        if not ctx.gripper_closed:
                            ...
                            # [T3] Ball-in-gripper check: if a ball is still
                            # visible close-up after the grip closed, the grasp
                            # failed — open gripper and retry from APPROACH.
                            # [v11] Use best_dist_cm (avg bbox+ellipse) for accuracy
                            if (target is not None and
                                    target.best_dist_cm <= CFG.dist_lock_cm * 1.5):
                                log.warning(
                                    "PICKUP: ball still visible at %.1f cm after "
                                    "grip — grasp failed, retrying APPROACH.",
                                    target.best_dist_cm)
...
def _(f, c):
    ...
    # [v9] IK retry gate: abort if >30 % of waypoints are unreachable
    ...
    if traj and none_count / len(traj) > 0.30:
        log.warning("PICKUP: %d/%d waypoints unreachable (>30%%). "
                    ...
                    none_count, len(traj))
        c.arm_traj = []   # signal abort (checked in update PICKUP handler)
    else:
        log.info("  trajectory: %d waypoints (%d unreachable).", len(traj), none_count)
...
class AutoPanTracker:
    ...
    def run(self):
        ...
        log.info("AutoPan v11 running. Press Q to quit.")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="AutoPan v11")
