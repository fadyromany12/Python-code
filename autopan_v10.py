"""
AutoPan Node — v10  (Raspberry Pi 4B/5 — Blueprint-Aligned)
============================================================
...
Run modes
---------
    python autopan_v10.py                   # normal
    python autopan_v10.py --no-serial       # vision only
    python autopan_v10.py --port /dev/ttyACM0
    python autopan_v10.py --imgsz 256       # slower Pi
    python autopan_v10.py --debug-hsv
    python autopan_v10.py --calibrate       # MUST re-run on Pi 5 with Pi camera
    python autopan_v10.py --int8            # use INT8 ONNX model
    python autopan_v10.py --target-count 5  # collect 5 balls

Step 1 (once):  python export_onnx.py
Step 2:         python autopan_v10.py
                    ...
                    if ctx.arm_step < len(ctx.arm_traj):
                        ...
                        if not ctx.gripper_closed:
                            ...
                            if (target is not None and
                                    target.dist_cm <= CFG.dist_lock_cm * 1.5):
                                log.warning(
...
_STATE_ID = {
    ...
    State.PICKUP: 4, State.LIFT: 5, State.DEPOSIT: 6, State.VERIFY: 7, State.SUCCESS: 8,
}
...
class ArtemisLink:
    ...
    Packet format (CSV + newline) — [v9 blueprint-exact]:
    ...
    def send(self, ctx: FSMContext):
        ...
        if ctx.state in (State.APPROACH, State.LOCK):
            ...
            self._prev_err_x = 0.0   # reset derivative: prevents spike on next move
...
class AutoPanTracker:
    ...
    def run(self):
        ...
        log.info("AutoPan v10 running. Press Q to quit.")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="AutoPan v10")
